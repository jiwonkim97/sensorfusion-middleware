...
const maxCnt = Math.floor(Math.min(camera1Data.length, camera2Data.length, camera3Data.length, camera4Data.length, lidar1Data.length))

export const GET = () => {
  const response = NextResponse.json([camera1Data[cnt], camera2Data[cnt], camera3Data[cnt], camera4Data[cnt], lidar1Data[cnt]])
  cnt += 30;
  cnt %= maxCnt;
